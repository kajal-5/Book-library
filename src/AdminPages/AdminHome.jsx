...
const AdminHome = () => {
  ...
  const filteredBooks = books.filter((book) => {
    ...
    if (selectedCategory !== "All") {
      ...
      const matchesName = book.name && book.name.toLowerCase().includes(query);
      const matchesDescription = book.description && book.description.toLowerCase().includes(query);
      ...
      try {
        ...
        if (data) {
          const requests = Object.entries(data)
            .filter(([_, notification]) => notification.type === "return_request" && notification.status === "pending")
